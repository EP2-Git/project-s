...
function run(label, command, args) {
  ...
  if (result.error) {
    fail(`${label} failed to start (${command}): ${result.error.message}`);
  }
  if (result.status !== 0) {
...
if (npmVersion.error) {
  fail(`unable to read npm version via ${npm}: ${npmVersion.error.message}`);
}
if (npmVersion.status !== 0) {
  fail(`unable to read npm version via ${npm}: exited with status ${npmVersion.status}`);
}
...
const requiredPaths = [
  ...
  "node_modules/@project-s/contracts",
  "node_modules/@project-s/application",
  "node_modules/@project-s/sdk",
  "node_modules/@project-s/mcp-server",
];
